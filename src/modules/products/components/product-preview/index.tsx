...
import { ProductHit } from "../../../store/components/product-list"

export type ProductPreviewProps = ProductHit & {}
  ...
  const formattedPrice = useMemo(() => {
    ...
    if (!variants || variants.length === 1) {
      const formattedAmount = formatAmount({
        amount: variants[0].prices[0].amount,
        region: cart?.region,
        includeTaxes: false,
        locale: "pl-PL",
      })
    ...
    const rawPrice = variants.reduce(
      (acc, variant) => {
        ...
        <div className=" flex flex-col justify-between h-full">
          ...
          <div className="text-base-regularh h-full py-2 flex flex-col justify-between text-stone-900">
            ...
            <div className="flex justify-between items-center">
              <span className="p-2">{formattedPrice}</span>
